...
- `src/types/fields/SelectFieldType.tsx`
- `src/features/data-editor/components/fields/SelectFieldConfig.tsx`
- `src/features/data-editor/components/DataTypeEditor.tsx`
...
- [x] `src/engine/core/TestEngine.ts` 作成（script/event/full 3モード対応）
- [x] ゲームループ実装（`src/engine/runtime/GameLoop.ts` — 固定タイムステップ 60fps、pause/resume）
- [x] GameEngine でサブシステム統合（input → world → triggers → events → camera → render）
- [x] シーン管理（`GameEngine` にマップロード/切り替えを統合）
- [x] 入力ハンドリング（`InputManager.ts` — キーマッピング、just-pressed、waiter方式）
- [x] テスト追加

**関連ファイル:**

- `src/engine/core/TestEngine.ts`
- `src/engine/core/TestEngine.test.ts`
...
- **ステータス:** [x] 完了（GameEngine に統合）
- **ブランチ:** main
- **PR:** -

**完了条件:**

- [x] `GameEngine.loadMap()` でマップロード（テクスチャ、オブジェクト、トリガーリセット）
- [x] マップ切り替え対応
...
- [x] GameEngine で pendingMapChange 消費（イベント完了後にマップロード + プレイヤーテレポート）
...
- [x] GameEngine で `updateAnimations(dt * 1000)` 呼び出し + UI proxy 注入
- [x] テスト追加（15テスト全通過）
...
- [x] `GameEngine` で AudioManager を初期化・注入（アセット名/ID 両対応）
...
- [x] T221b: GameEngine + ScriptRunner に統合（Tween 注入）
...
- [x] GameEngine.update() で `processWaiters()` 呼び出し
...
#### [T224f] Integrate lifecycle dispatch into GameEngine
...
- [ ] GameEngine の update ループで dispatchInput / dispatchUpdate を呼ぶ
...
- [x] GameEngine: cancel ボタンでメニュー起動、start 時にスタートスクリプト実行
...
- [x] ScriptPickerModal: 検索 + スクリプト名/callId/説明表示
- [x] ScriptActionBlock: ドロップダウン → ボタン + モーダル
...
- [x] Script.icon フィールド追加
- [x] IconPicker: RPG 向けプリセット 60+ アイコン、カテゴリ別グリッド
- [x] ScriptSettingsPanel でアイコン選択
- [x] ScriptPickerModal でアイコン表示
...
右カラム（`w-inspector`、300px）が縦にも狭く、チップグリッド＋プロパティフォームを埋め込む形（固定パネル＋独立スクロールのグリッド）を試したが、それでも窮屈だったため、design.md/requirements.mdの3カラム構成から意図的に外れてモーダルに切り出す方針に変更。

- [x] チップ一覧タブは「チップ一覧を編集」ボタンのみに簡略化。クリックすると`Modal`（`size="xl"`, 800px）が開き、左にチップグリッド・右に選択中チップの`ChipPropertyEditor`を並べたレイアウトで編集する（グリッド・プロパティともに`overflow-auto`で個別スクロール、モーダル全体は`max-h-[70vh]`）
- [x] グリッド上のクリック挙動を変更: 未選択のチップをクリック＝選択、**既に選択中のチップを再度クリック**＝選択を維持したまま`passable`（通行可能）をその場でトグル。右クリックでのトグルは廃止（左クリックのみに統一）

**注記:**

- モーダルはチップセットが存在する間だけ`open`可能（`selectedChipIndex !== null`ではなく`chipset !== null`で判定。モーダルを開いてから中でチップを選ぶ流れになったため）
...
`src/engine/` 以下のゲームエンジン（TestEngine / EventRunner / actions 等）の
Lite/Full テンプレート対応。
...
- [x] `.husky/commit-msg` に実行権限(755)を付与
- [x] `.husky/pre-commit` に実行権限(755)を付与
...
- [x] `src/app/page.tsx` の Get Started ランディング画面を削除
- [x] `redirect('/data')` でルートアクセス時にデータ管理ページへ遷移
...
- [x] 「なぜ作ったの？」を具体的な原体験（WOLF RPGエディター経験→着想）に書き直し
- [x] 「どう作ったの？」にAI駆動開発の失敗談・仕様駆動開発への転換理由・現在も続く課題感を加筆（Undo/Redo実装時の実例を含む）
- [x] 「今後どうしたいか」セクションを新設
...
| 1     | 型定義・基盤                   | 🔶 一部未着手 (20/30) | 残: autoSave refactor, per-page undoのIndexedDB永続化（インメモリ実装・全ページ展開は完了）, ShortcutHelpModal, ValidationManager, InlineError, useValidation, SearchModal, ReferenceSearchModal, ClipboardManager, useClipboard, 汎用PropertyPanel（全てPolish/必要時実装） |
