...
**備考:** 元の SelectableComponent を廃止。Navigation + NavigationItem（opt-in）+ NavigationCursor の3分割に再設計。NavigationItem の `onSelectActions` は T184 ActionComponent に移行予定

---

#### [T184] [P] Implement ActionComponent
...
- [ ] `src/types/ui/components/ActionComponent.ts` 作成
- [ ] `UIActionEntry` interface（id, name, blocks: SerializedAction[]）
- [ ] `actions: UIActionEntry[]` プロパティ
- [ ] serialize/deserialize/clone 実装（structuredClone 使用）
- [ ] テスト追加
- [ ] NavigationItemComponent から `onSelectActions` を削除
- [ ] NavigationItemComponent のテスト更新
- [ ] `register.ts` と `index.ts` を更新（12コンポーネント登録）

**関連ファイル:**

- `src/types/ui/components/ActionComponent.ts` + `.test.ts`
- `src/types/ui/components/NavigationItemComponent.ts`（変更: onSelectActions 削除）

**備考:** 設計再検討で復活。onClick/onSelect/onHover 等のアクションを ID 付きリストで保持。NavigationItem は選択時に同オブジェクトの ActionComponent の `onSelect` アクションを発火する
...
#### [T187] TemplateControllerComponent — Phase 15 で実装

- **ステータス:** [ ] 後回し（Phase 15）

**備考:** テンプレート保存時に自動追加されるコンポーネント。Phase 15 の T196/T197 と合わせて実装
...
> **備考:** T189 を拡張し Phase 18 の UIRenderer (T214) を統合。WebGL で全 11 UIComponent を描画。
...
- [ ] `src/stores/uiEditorSlice.ts` 作成
- [ ] UICanvas CRUD（uiCanvases, selectedCanvasId）
- [ ] UIObject CRUD（addObject, updateObject, deleteObject, reparentObject）
- [ ] UIComponent 追加/削除/更新（addComponent, removeComponent, updateComponent）
- [ ] UIFunction CRUD（addFunction, updateFunction, deleteFunction）
- [ ] UITemplate CRUD（uiTemplates, addUITemplate, updateUITemplate, deleteUITemplate）
- [ ] エディタ状態（viewport, selectedObjectIds, showGrid, gridSize, snapToGrid）
- [ ] 左パネルモード（leftPanelMode: 'canvasList' | 'elements' | 'templates' | 'functions'）
- [ ] テスト追加

**関連ファイル:**

- `src/stores/uiEditorSlice.ts`
- `src/stores/uiEditorSlice.test.ts`

**備考:** 既存 `uiSlice.ts`（SaveStatus のみ）とは別スライス。stores/index.ts で統合
...
- [ ] `src/app/(editor)/ui/screen/page.tsx` 作成
- [ ] ThreeColumnLayout 使用
- [ ] 左パネル: ドロップダウン切替（画面一覧 / エレメント / テンプレート / ファンクション）
- [ ] 中央: UICanvas（T189c）
- [ ] 右パネル: UIPropertyPanel（T191）/ UIFunctionEditor（T197a）切替
- [ ] テスト追加

**関連ファイル:**

- `src/app/(editor)/ui/screen/page.tsx`
- `src/app/(editor)/ui/screen/page.test.tsx`
...
- [ ] WebGL コンテキスト初期化（twgl.js、MapCanvas と同パターン）
- [ ] `useUIViewport` — ズーム/パン（ホイール + ミドルクリック/Space+ドラッグ）
- [ ] 解像度プレビュー枠（ゲーム画面範囲を枠で表示）
- [ ] グリッド表示（スナップ用）
- [ ] DOM オーバーレイコンテナ（選択枠・ハンドル用、T194 で使用）
- [ ] テスト追加
...
- `src/features/ui-editor/hooks/useUIViewport.ts`
- `src/features/ui-editor/hooks/useUIViewport.test.ts`
...
- [ ] `src/features/ui-editor/renderer/UIRenderer.ts` 作成
- [ ] UIObject ツリーを走査して描画順（Z-order + 親子階層）でソート
- [ ] **Image**: テクスチャ付きクワッド描画（tint, opacity, nine-slice 対応）
- [ ] **Text**: Canvas2D → テクスチャ変換でテキスト描画（fontSize, fontId, color, align, lineHeight）
- [ ] **Shape**: シェーダーで矩形/楕円/多角形描画（fill, stroke, cornerRadius）
- [ ] **FillMask**: 方向と fillAmount に基づくクリッピング（シェーダー）
- [ ] **ColorMask**: ブレンドモード切替で子に色乗算
- [ ] RectTransform → ワールド座標変換（親子階層、anchor, pivot, rotation, scale 考慮）
- [ ] テスト追加

**関連ファイル:**

- `src/features/ui-editor/renderer/UIRenderer.ts`
- `src/features/ui-editor/renderer/UIRenderer.test.ts`
- `src/features/ui-editor/renderer/shaders.ts`
...
- [ ] **LayoutGroup**: direction/spacing/alignment に基づく子オブジェクトの transform 自動計算
- [ ] **GridLayout**: columns/spacingX/spacingY に基づく子オブジェクトのグリッド配置計算
- [ ] **Navigation**: 子の NavigationItem を収集し、方向キーでフォーカス移動するロジック
- [ ] **NavigationItem**: 選択時に同オブジェクトの ActionComponent.onSelect を発火
- [ ] **NavigationCursor**: フォーカス中アイテムの位置 + offset に基づく配置計算
- [ ] **Action**: UIActionEntry の blocks 実行ランタイム（SerializedAction → 実行）
- [ ] **Animation**: inlineTimeline の TweenTrack に基づくプロパティ補間ロジック（エディタプレビュー用）
- [ ] テスト追加
...
- `src/features/ui-editor/renderer/actionRunner.ts`
...
- [ ] 階層表示（parentId ベース）
- [ ] ドラッグ&ドロップで親子変更
- [ ] 右クリックメニュー（追加 / 複製 / 削除 / テンプレート保存）
- [ ] 選択と UICanvas の同期（ツリー選択 ↔ キャンバス選択）
- [ ] テスト追加
...
- [ ] UIComponentRegistry から使用可能コンポーネント一覧取得
- [ ] カテゴリ分類（Visual / Mask / Layout / Navigation / Animation）
- [ ] クリックまたはドラッグで選択オブジェクトにコンポーネント追加
- [ ] テスト追加
...
- [ ] RectTransform 編集（x, y, width, height, anchorX/Y, pivotX/Y, rotation, scaleX/Y）
- [ ] アタッチ済みコンポーネント一覧表示
- [ ] 各コンポーネントの renderPropertyPanel() を表示
- [ ] コンポーネント追加/削除ボタン
- [ ] テスト追加
...
- [ ] キャンバス上のクリックで UIObject 選択（ヒットテスト）
- [ ] Shift+クリックで複数選択
- [ ] 選択ハイライト表示（DOM オーバーレイ）
- [ ] ツリーとの選択同期
- [ ] テスト追加
...
- [ ] `src/features/ui-editor/components/TransformHandles.tsx` 作成（DOM オーバーレイ）
- [ ] 移動ハンドル（ドラッグで x/y 変更）
- [ ] リサイズハンドル（8方向、ドラッグで width/height 変更）
- [ ] 回転ハンドル（ドラッグで rotation 変更）
- [ ] グリッドスナップ対応
- [ ] テスト追加
...
- [ ] 9ポイントプリセット（左上/上/右上/左/中央/右/左下/下/右下）
- [ ] ストレッチプリセット（横/縦/全方向）
- [ ] ビジュアルプレビュー（UIPropertyPanel 内に表示）
- [ ] テスト追加
...
- [ ] 選択オブジェクトをテンプレート化（子オブジェクト含む深いコピー）
- [ ] TemplateControllerComponent 自動追加（Phase 14 で後回しにした T187 をここで実装）
- [ ] 名前付けダイアログ
- [ ] ストアに保存
- [ ] テスト追加
...
- `src/types/ui/components/TemplateControllerComponent.ts`（T187 実装）
...
- [ ] 全 ID 再生成（衝突回避）
- [ ] 配置位置指定
- [ ] TemplateControllerComponent の args → オブジェクトプロパティ反映
- [ ] テスト追加
...
- [ ] 左パネル「ファンクション」モード: ファンクション一覧表示（show/hide はデフォルト提供）
- [ ] ファンクション追加/削除
- [ ] 右パネル: 選択ファンクションのブロック編集（イベントエディタのブロック UI を再利用）
- [ ] 引数定義 UI（TemplateArg）
- [ ] テストボタン（▶）でファンクション単体実行（キャンバスにリアルタイム反映）
- [ ] テスト追加

**関連ファイル:**

- `src/features/ui-editor/components/UIFunctionList.tsx`
- `src/features/ui-editor/components/UIFunctionEditor.tsx`
- 各 `.test.tsx`

---

#### [T197b] [US14] Create ActionComponent block editor
...
- [ ] UIPropertyPanel 内で ActionComponent のアクション一覧を表示
- [ ] アクション追加/削除（id + name 設定）
- [ ] 各アクションのブロック編集（UIFunction と同じブロック UI を共有）
- [ ] ブロック内容: ロジック（条件分岐/ループ）、ウェイト、UIオブジェクトプロパティ操作、他アクション発火
- [ ] テスト追加

**関連ファイル:**

- `src/features/ui-editor/components/ActionEditor.tsx`
- `src/features/ui-editor/components/ActionEditor.test.tsx`

**備考:** UIFunction と ActionComponent は同じブロック編集 UI を共有。差異は引数の有無と呼び出し元（Canvas レベル vs Object レベル）
