...
#### [T218] [P] Implement UIAPI (UICanvasProxy / UIObjectProxy / UIComponentProxy)
...
- [ ] `UI[canvasName]` → `UICanvasProxy` — キャンバスへのプロキシアクセス
- [ ] UIFunction 自動メソッド生成 — `UI["menu"].show()`, `UI["menu"].hide({ fade: true })`
- [ ] `UICanvasProxy.getObject(name)` → `UIObjectProxy`
- [ ] `UICanvasProxy.spawn(templateId, args?)` → `UIObjectProxy`
- [ ] `UICanvasProxy.despawn(objectId)`
- [ ] `UIObjectProxy` — Transform直接アクセス（x, y, width, height, scaleX, scaleY, rotation, visible）
- [ ] `UIObjectProxy.get(component, property)` / `.set(component, property, value)`
- [ ] `UIObjectProxy.getComponent(type)` → `UIComponentProxy`
- [ ] `UIObjectProxy.getChild(name)` / `.getChildren()`
- [ ] `UINavigationProxy.select()` → `Promise<string>`（itemId を返す）
- [ ] `UINavigationProxy.focus(index)`
- [ ] `UIAnimationProxy.play(name, options?)` / `.stop()`
- [ ] `UIRepeaterProxy.setData(items)` / `.getItems()`
- [ ] テスト追加
...
**備考:** ScriptAPI.UI として公開。UIFunction のアクションでできることはすべてスクリプトからも直接可能。design.md の UICanvasProxy / UIObjectProxy / UIComponentProxy 参照
