...
| 用語                 | 定義                                     | 用途                               |
| -------------------- | ---------------------------------------- | ---------------------------------- |
| **フィールドセット** | 複数フィールドの再利用可能なテンプレート | データタイプのフィールド構成に使用 |
| **クラス**           | 変数で使うカスタム構造体                 | 変数のタイプとして使用             |

**フィールドセットとクラスの違い:**

```
フィールドセット                    クラス
├─ データタイプのフィールドに適用    ├─ 変数のタイプとして使用
├─ 参照方式（変更が全体に反映）     ├─ 値方式（個別にデータを持つ）
└─ 例: 基本ステータス、ドロップ      └─ 例: パーティメンバー情報、座標
```
...
データ・フィールドセット・クラス・プレハブなどが他から参照されている場合の削除フロー:
...
フィールドの組み合わせをテンプレートとして管理。複数のデータタイプで共通して使うフィールドを再利用可能。

### フィールドセット仕様

- **適用方式**: 参照方式（セット変更時に全データタイプに反映）
- **表示方式**: データタイプ編集画面でインライン展開（グループ化して区別）
- **フィールド編集**: セット由来のフィールドは表示のみ、編集はフィールドセットページで行う
- **フィールドタイプ**: データ設定と同じ（数値/文字列/選択肢/データ参照など）

### フィールドセット作成フロー

```mermaid
flowchart TD
    A[フィールドセットページ] --> B[+ 新規追加ボタン]
    B --> C[セット名入力]
    C --> D[セット一覧に追加]
    D --> E[セット編集パネル表示]
    E --> F[+ フィールド追加]
...
| ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-01-27 | 初版作成                                                                                                                                                                                                                                                                    |
| 2026-01-27 | フィールドセット/変数/クラス/マップ編集/マップデータ/プレハブ/イベントテンプレート画面詳細追加、キャンバス共通操作追加                                                                                                                                                      |
